...
with open("input.txt","r") as f:
    ...
    for line in f:
        ...
        to_stack = int(line[-1])
        crates_to_move = inventory[from_stack][-n:].copy()
        del inventory[from_stack][-n:]
        inventory[to_stack].extend(crates_to_move)
        print(f"[MOVING] Move {crates_to_move} from stack {from_stack} to stack {to_stack}")
    ans=""
    for stack in inventory.keys():
        ans+=inventory[stack][-1]
    print(ans)
